  ...
  Meteor.subscribe("meteor_autoupdate_clientVersions", appId, {
    ...
    onReady() {
      function checkNewVersionDocument(doc) {
        // console.log('-> version', autoupdateVersionsCordova.version);
        if (doc.version !== autoupdateVersionsCordova.version) {
          newVersionAvailable({ newVersion: doc.version, currentVersion: autoupdateVersionsCordova.version  });
        }
      }
...
function newVersionAvailable(versionConfig) {
  // console.log('-> newVersionAvailable', versionConfig);
  if (window.MeteorCapacitor != null && window.MeteorCapacitor.newVersionAvailable != null) {
    window.MeteorCapacitor.newVersionAvailable(versionConfig);
  }
}
